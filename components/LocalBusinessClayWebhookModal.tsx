...
import { sendLocalBusinessesToClay, getClayWebhookUrl } from '@/lib/clay-webhook';

interface LocalBusinessClayWebhookModalProps {
  isOpen: boolean;
  onClose: () => void;
  businesses: LocalBusinessDisplay[];
}

export default function LocalBusinessClayWebhookModal({ isOpen, onClose, businesses }: LocalBusinessClayWebhookModalProps) {
  ...
  const handleSend = async () => {
    ...
    setIsSending(true);
    try {
      const result = await sendLocalBusinessesToClay(businesses, webhookUrl);
      if (result.success) {
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div className="p-4 space-y-4">
          <p className="text-sm text-gray-600">
            Enter your Clay.com webhook URL to send {businesses.length} business{businesses.length !== 1 ? 'es' : ''} directly to Clay.
            <span className="block mt-1 text-xs text-gray-500">
              Each business will be sent as a separate webhook request (one business = one row in Clay).
            </span>
          </p>

          <div>
            <label htmlFor="clay-webhook-url-business" className="block text-sm font-medium text-gray-700 mb-1">
              Clay.com Webhook URL <span className="text-red-500">*</span>
            </label>
            <input
              id="clay-webhook-url-business"
              type="url"
              value={clayUrl}
        ...
        <div className="flex items-center justify-end gap-2 p-4 border-t border-gray-200">
          ...
          <button
            onClick={handleSend}
            disabled={isSending || !clayUrl.trim() || businesses.length === 0}
